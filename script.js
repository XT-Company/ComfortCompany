...
let profileData = {
    ...
    email: '',
    companyName: '',
    avatarUrl: '' // Base64 или URL картинки профиля
};

// Константы для Google API
const DRIVE_APP_DATA_URL = 'https://www.googleapis.com/drive/v3/files';
const PROFILE_FILE_NAME = 'profile_data.json';

/* --- INITIALIZATION & AUTH --- */
window.onload = function() {
    // Инициализация кнопки авторизации Google
    document.getElementById('btn-google-login').addEventListener('click', handleGoogleLogin);
    
    // Перехват отправки формы завершения регистрации
    document.getElementById('profileSetupForm').addEventListener('submit', handleProfileSetupSubmit);
    
    // Обработка загрузки логотипа/аватара
    document.getElementById('setupAvatarInput').addEventListener('change', handleAvatarUpload);
};

function handleGoogleLogin() {
    const client = google.accounts.oauth2.initTokenClient({
        client_id: 'YOUR_GOOGLE_CLIENT_ID.apps.googleusercontent.com', // Замени на свой Client ID
        ...
        callback: async (tokenResponse) => {
            if (tokenResponse && tokenResponse.access_token) {
                ...
                // 1. Пытаемся загрузить данные из Google Drive appDataFolder
                const loadedData = await loadProfileFromGoogleDrive();
                ...
                if (loadedData) {
                    ...
                    // 2. Если файла нет, берем базовый инфо из аккаунта и просим дозаполнить (Возраст, Компания)
...
async function fetchGoogleUserProfile() {
    try {
        ...
        // Предзаполняем объект из Google Account
        profileData.firstname = userInfo.given_name || '';
        profileData.lastname = userInfo.family_name || '';
        profileData.email = userInfo.email || '';
        profileData.avatarUrl = userInfo.picture || ''; // Дефолтная аватарка Google
        
        // Показываем форму дозаполнения профиля (Возраст, Компания, кастомный логотип)
        showSetupProfileWindow();
    } catch (error) {
        console.error('Error fetching Google profile:', error);
    }
}

function showSetupProfileWindow() {
    document.getElementById('authWindow').style.display = 'none';
    document.getElementById('setupProfileWindow').style.display = 'block';
    
    // Заполняем инпуты тем, что уже получили от Google
    document.getElementById('setupFirstname').value = profileData.firstname;
    document.getElementById('setupLastname').value = profileData.lastname;
    if (profileData.avatarUrl) {
        document.getElementById('setupAvatarPreview').innerHTML = `<img src="${profileData.avatarUrl}" style="width:100%; height:100%; border-radius:50%; object-fit:cover;">`;
    }
}

/* --- AVATAR HANDLER (Base64) --- */
function handleAvatarUpload(e) {
    ...
    reader.onload = function(event) {
        profileData.avatarUrl = event.target.result; // Сохраняем как Base64 String
        document.getElementById('setupAvatarPreview').innerHTML = `<img src="${profileData.avatarUrl}" style="width:100%; height:100%; border-radius:50%; object-fit:cover;">`;
    };
...
async function handleProfileSetupSubmit(e) {
    ...
    profileData.age = age;
    // Название компании (можно добавить инпут во фреймворк HTML, если его там еще нет)
    profileData.companyName = document.getElementById('setupCompanyName')?.value || 'My Personal Company'; 
    
    // Сохраняем в облако Google Drive
...
/* --- GOOGLE DRIVE APPDATA STORAGE LOGIC --- */

// Загрузка файла настроек из скрытой папки приложения
async function loadProfileFromGoogleDrive() {
    try {
        // Поиск файла по имени в пространстве appDataFolder
        const searchUrl = `${DRIVE_APP_DATA_URL}?q=name='${PROFILE_FILE_NAME}' and 'appDataFolder' in parents&spaces=appDataFolder`;
        ...
        if (searchResult.files && searchResult.files.length > 0) {
            ...
            // Скачиваем содержимое файла (alt=media)
            const fileResponse = await fetch(`${DRIVE_APP_DATA_URL}/${fileId}?alt=media`, {
                headers: { 'Authorization': `Bearer ${accessToken}` }
            });
            return await fileResponse.json();
        }
        return null; // Файла еще нет
    } catch (error) {
...
// Сохранение/Обновление файла настроек на Google Drive
async function saveProfileToGoogleDrive() {
    try {
        ...
        if (searchResult.files && searchResult.files.length > 0) {
            // Файл существует -> Будем обновлять его (PATCH метод)
            const fileId = searchResult.files[0].id;
            url = `https://www.googleapis.com/upload/drive/v3/files/${fileId}?uploadType=multipart`;
            method = 'PATCH';
        } else {
            // Файла нет -> Создаем новый (POST метод)
        ...
        console.log('Profile successfully synced with Google Account!');
    } catch (error) {
...
function applyProfileToUI() {
    ...
    if (sidebarAvatar && profileData.avatarUrl) {
        sidebarAvatar.innerHTML = `<img src="${profileData.avatarUrl}" style="width:100%; height:100%; border-radius:50%; object-fit:cover;">`;
    }
    
    // Обновляем почту на странице писем
    const userEmailSpan = document.getElementById('user-email');
    if (userEmailSpan) {
        userEmailSpan.textContent = profileData.email;
    }
    
    // Обновляем данные компании на вкладке Company
    const compName = document.getElementById('compDashboardName');
    if (compName) {
        compName.textContent = profileData.companyName;
    }
}
